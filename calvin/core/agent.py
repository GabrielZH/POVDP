...
class MemoryAgent(Agent):
    ...
    def _step(self, episodes: List[Episode], save_pred=False, **kwargs):
        ...
        print(f"pred actions: {self.actions}")
    ...
    def rollout(self, env: Env, episode_info: dict = None, train: bool = True, experiences: ExperienceManager = None, stats: Stats = None, manual=False, save_pred=False):
        ...
        while True:
            ...
            state_list.append(obsv['poses'])
            episode.push(action, obsv, reward, done, info)
            if done:
                if not torch.all(torch.eq(obsv['poses'], episode_info['target'].int())):
                    print(f"state history: {state_list}")
                    print(f"action history: {action_list}")
                break
        if save_pred:
            episode.save()
        stats.add_all(self.handler.stats(info))
