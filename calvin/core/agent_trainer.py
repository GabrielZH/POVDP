...
class AgentTrainer:
    ...
    def _init_vecenv(self, env_config, meta, init_env, n_envs):
        self.vecenv_train = VecEnv(
            [lambda: init_env(meta, i_env=i_env, split="train", index=0, **env_config) for i_env in range(n_envs)])
        self.vecenv_val = VecEnv(
            [lambda: init_env(meta, i_env=i_env, split="val", index=1, **env_config) for i_env in range(n_envs)])
