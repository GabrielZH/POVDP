...
class Object:
    ...
    def embedding(self):
        ...
        pad = 5
        # embeddings = np.pad(embeddings[0], [(0,), (pad,), (pad,)], mode='constant')
...
class Scene:
    ...
    def __init__(self, scene_name: str, scale: Union[float, int], camera_params_path: str,
    ...
    def target_coords_rgb_depth(self, image_names, target_obj_name):
        camera = self.camera
        target_coords = list()
        target_rgb = list()
        target_depth = list()
        for i, image_name in enumerate(image_names):
            ...
            for j, obj in enumerate(node.objects):
                if obj.object_class.name == target_obj_name:
                    depth = obj.depth()
                    H, W = depth.shape
                    # if H * W < 50: continue
                    x1, y1, x2, y2 = obj.bbox

                    rescale_factor_x = camera.rescale_width / camera.width
                    rescale_factor_y = camera.rescale_height / camera.height

                    target_us, target_vs = np.meshgrid(
                        np.linspace(x1 / rescale_factor_x, x2 / rescale_factor_x, W), 
                        np.linspace(y1 / rescale_factor_y, y2 / rescale_factor_y, H)
                    )
                    if target_us.size == 0 or target_vs.size == 0:
                        print(f"Empty target_us or target_vs for bbox: ({x1}, {y1}), ({x2}, {y2})")
                        continue
                    target_xs = (target_us - camera.c_x) / camera.f_x * (1)
                    target_ys = (target_vs - camera.c_y) / camera.f_y * (1)
                    target_mesh_xs, target_mesh_ys = undistort_mesh(
                        target_xs, 
                        target_ys, 
                        camera.distort_coeffs
                    )

                    world_coords = get_world_coord_from_mesh(
                        target_mesh_xs, 
                        target_mesh_ys, 
                        depth, 
                        node.R, 
                    ...
                    rgb = obj.rgb()
                    target_coords.append(world_coords.reshape(-1, world_coords.shape[-1]))
                    target_rgb.append(rgb.reshape(-1, rgb.shape[-1]))
                    target_depth.append(depth.reshape(-1))

        target_coords = np.concatenate(target_coords, axis=0) if target_coords else np.array(target_coords)
        target_rgb = np.concatenate(target_rgb, axis=0) if target_rgb else np.array(target_rgb)
        target_depth = np.concatenate(target_depth, axis=0) if target_depth else np.array(target_depth)

        return target_coords, target_rgb, target_depth
...
class ImageNode:
    ...
    objects: List['Object']
    world_coords: Optional[np.ndarray]
    is_valid_coords: Optional[np.ndarray]
    ...
    def embedding(self):
        if self.scene.embeddings is not None:
            return self.scene.embeddings[self.image_index]
        rgb = torch.from_numpy(
            self.scene.rgb_images[self.image_index]
        ).to('cuda')
        resnet = PretrainedResNetShort(freeze=True, device='cuda')
        resnet.eval()
        return resnet(
            rearrange(
                rgb, "h w f -> f h w"
            ).float() / 255
        ).cpu().type(torch.float16)
