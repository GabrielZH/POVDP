...
class RGBLoader:
    ...
    def __getitem__(self, indices: List):
        return np.array(
            [plt.imread(
                os.path.join(
                    self.dir, self.image_names[index]
                ) 
...
class DepthLoader:
    ...
    def __getitem__(self, indices: Union[int, List]):
        if isinstance(indices, int):
            path = os.path.join(
                self.dir, change_image_type(self.image_names[indices], ImageType.DEPTH)
            )
            return np.array(cv2.imread(path, cv2.IMREAD_ANYDEPTH))
        depth_images = list()
        for index in indices:
            path = os.path.join(
                self.dir, change_image_type(self.image_names[index], ImageType.DEPTH)
            )
            depth_images.append(
