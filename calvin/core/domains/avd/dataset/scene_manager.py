...
class AVDSceneManager(dict):
    def __init__(
        ...
        scenes_in_use = {**self.train, **self.val}.keys()
        self.split = {'train': self.train, 'val': self.val}
        for scene_name in self.scene_names:
            if scene_name not in scenes_in_use:
                self.pop(scene_name)
    ...
    def reset_targets(self, target_name=None):
        ...
        for scene_name in self.scene_names:
            ...
            target_classes = [
                scene.object_classes.get_by_name(target_name)
            ] if target_name else scene.object_classes.values()
