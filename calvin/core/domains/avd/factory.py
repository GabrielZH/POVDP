...
def init_avd_meta(
        ...
        avd_workers=None, 
        segment_config_path=None,
        **kwargs
) -> AVDPoseMDPMeta:
    scenes = AVDSceneManager(
        data_dir=os.path.join(avd_data, "src"), 
        scene_resize=resize, 
        target=target, 
        in_ram=in_ram, 
        target_size_ratio=target_size_ratio, 
        target_dist_thresh=target_dist_thresh, 
        avd_workers=avd_workers
    )
    return AVDPoseMDPMeta(
        scenes=scenes, 
        map_bbox=map_bbox, 
        map_res=map_res, 
        ori_res=ori_res, 
        segment_config_path=segment_config_path
    ) if ori_res else AVDPosMDPMeta(
        scenes=scenes, 
        map_bbox=map_bbox, 
        map_res=map_res,
        segment_config_path=segment_config_path
