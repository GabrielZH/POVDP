...
class Episode:
    ...
    def get(self, step=None, inference=False, include_pred=False):
        ...
        curr_info, past_seq_info, future_seq_info, full_seq_info = \
            self.handler.combine_seq_info(
                episode_info, 
                self.handler.postproc_obsvs(
                    dict(self.obsv_io[index]), 
                    episode_info
                ),
                self.handler.postproc_trans(
                    dict(self.trans_io[index])
                ), step, inference, start_step=start_step)
...
class ExperienceManager(ExperienceManagerBase):
    def __init__(
        ...
        self.samples_info = JSONIO(
            os.path.join(save_dir, "samples.json")
            if save_dir else None
        )
        self._index = int(sorted(list(self.samples_info.keys()))[-1]) \
            if len(self.samples_info) else 0
    ...
    def get_io(self, save_dir, name, in_mem: bool):
        ...
        return DictIO(path=save_path) \
            if in_mem or not save_path else MultiFileIO(save_path, cash_size=self.cash_size)
    ...
    def collect_grid_demos(self, env: Env, n_envs: int, n_episodes_per_env: int):
        if len(self):
        ...
        for i in range(n_envs):
            sys.stdout.write(f"\r--- {i + 1} / {n_envs} envs")
            sys.stdout.flush()

            n_episode_info, n_init_states, n_init_obsvs, \
                n_targets, n_opt_action_trajs = env.reset_grid(
                n_episodes_per_env=n_episodes_per_env)

            for j in range(n_episodes_per_env):
                episode = self.add_episode({**n_episode_info[j], "is_expert": True}, n_init_obsvs[j])
                env.count_steps = 0
                env.total_rewards = 0
                env.feature_map = n_init_obsvs[j]['feature_map']
                env.state = n_init_states[j]
                env.target = n_targets[j]

                for action in n_opt_action_trajs[j]:
    ...
    def collect_avd_demos(self, env: Env, n_episodes: int):
        if len(self):
            raise Exception("This experiment has already been run. "
                            "Please rename the save directory of your previous experiment if you want a rerun. "
                            "If you want to overwrite, pass --clear as an argument.")
        ...
        for i in range(n_episodes):
            ...
            sys.stdout.flush()
            episode_info, obsv, opt_actions = env.reset_avd()
