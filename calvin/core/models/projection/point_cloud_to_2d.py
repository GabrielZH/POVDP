    ...
    def __init__(
        ...
        if use_resnet:
            ...
            self.rgb_conv_net = nn.Sequential(
                *((nn.GroupNorm(8, 64),) if use_group_norm else ()),
            ...
            self.rgb_conv_net = nn.Sequential(
                *((nn.GroupNorm(8, self.i),) if use_group_norm else ()),
                nn.Conv2d(in_channels=self.i, out_channels=h, dilation=(2, 2),
                          kernel_size=(3, 3), stride=(1, 1), padding=(2, 2), bias=False),
                nn.Dropout(dropout),
                nn.Mish(),
                *((nn.GroupNorm(8, h),) if use_group_norm else ()),
        ...
        self.pre_dot_layer = nn.Conv2d(in_channels=self.i, out_channels=dot_channels * h,
