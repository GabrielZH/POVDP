...
class DatasetNormalizer:
    ...
    def __init__(self, dataset, normalizer, path_lengths=None, n_bits=None):
        ...
        for key, val in dataset.items():
            try:
                # bit normalization for discrete fields
                if self.normalizer_name == 'BitNormalizer':
                    if key in {
                        'actions', 
                        'states', 
                        'next_states'
                    }:
                        self.normalizers[key] = normalizer(val, n_bits=n_bits)
                # other normalization (e.g., limit norm) for continuous fields
                else:
                    if key in {
                        'feature_maps', 'next_feature_maps', 
                        'obsv_maps', 'next_obsv_maps', 
                    }:
...
class Normalizer:
    ...
    def __init__(self, X):
        ...
        if X.ndim == 2:  # temporal
            self.mins = X.min(axis=0, keepdims=True)
            self.maxs = X.max(axis=0, keepdims=True)
        elif X.ndim == 4:
            self.mins = X.min(axis=(0, 2, 3), keepdims=True)
            self.maxs = X.max(axis=(0, 2, 3), keepdims=True)
...
class BitNormalizer(Normalizer):
    '''
    '''
    ...
    def normalize(self, x):
        ...
        x = np.right_shift(
            ...
            np.arange(self.n_bits)).astype(dtype)
        x = np.mod(x, 2)
        x = x * 2 - 1
        return x
    ...
    def unnormalize(self, x):
        ...
        x = np.sum(x * (2 ** np.arange(self.n_bits)), axis=-1)
        return x
