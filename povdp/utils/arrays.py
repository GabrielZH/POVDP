...
def to_device(x, device=DEVICE):
	if torch.is_tensor(x):
		...
		return [
			[bx.to(device) for i, bx in enumerate(_x)]
...
def batchify(batch):
	...
	'''
	fn = lambda x: to_torch(x[None], dtype=torch.int)

	batched_vals = []
	for field in batch._fields:
		val = getattr(batch, field)
		if type(val) is dict:
			val = apply_dict(fn, val)
		elif type(val) is list:
			val = [
				[fn(_v) for i, _v in enumerate(v)] 
...
def apply_dict(fn, d, *args, **kwargs):
	...
	multi_vals = {
		k: [fn(v, *args, **kwargs) for v in vs] 
		for k, vs in d.items() 
		if type(vs) is tuple or type(vs) is list
	}
