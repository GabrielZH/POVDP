...
def append_dims(x, target_n_dim):
    """
    Append dimensions to the end of a tensor till it gets n_dims dimensions
    """
    if target_n_dim < x.ndim:
        raise ValueError(
            f"input has less dims ({x.ndim}) than target n_dims ({target_n_dim})"
        )
    dim_to_append = target_n_dim - x.ndim
    return x[(...,) + (None,) * dim_to_append]